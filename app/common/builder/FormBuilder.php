...
class FormBuilder
{
     ...
     * @return $this|array
     */
    public function addDiscountText($name = '', $title = '', $tips = '', $default = '', $group = [], $extra_attr = '', $extra_class = '', $placeholder = '', $required = false)
    {
        $item = [
            'type'        => 'discount_text',
            'name'        => $name,
        ...
        if (is_array($groups) && !empty($groups)) {
            ...
            foreach ($groups as &$group) {
                foreach ($group as $key => $item) {
                    ...
                    // 转换首字母大写，找到对应方法并调用
                    $group[$key] = call_user_func_array([$this, 'add'.ucfirst(toCamelCase($type))], $item);
                }
            }
