...
function userinit(){
  ...
  $('.go-repassword').on('click', function(event){
    ...
    var re_password = $("input[name='password2']").val();
    _this.html(iconspin+deft)
  ...
  $(document).on('click', '[etap="star"]', function (event) {
      ...
      _this.html(iconspin)
      $.post(collectUrl, {id,mid}, function(data) {
          if (data.code === 1) {
            _this.html(deft)
            const Toast = Swal.mixin({
              toast: true,
            ...
            Toast.fire({
              ...
              if (location.href.indexOf('myfav') !== -1 && data.msg.indexOf('取消') !== -1) {
                setTimeout(function () {
                    location.reload();
                },1000)
              }
  ...
  $('.click-qiandao').on('click', function(){
    ...
    $.post(signInUrl,{action: 'user_qiandao'},
          function (data) {
              if (data.code == 1) {
                _this.html(deft)
...
function to_pay_post(pay_id,type_id,post_id,price){
...
// 文章购买函数
function to_yecpay_post(pay_id,type_id,post_id,price){
    var _this = $(this)
...
function widget_ri(){
  ...
  $(".click-pay").on("click",function(){
    var _this = $(this)
    var deft = _this.html()
    var post_id = $(this).data("postid") || $("input[name='pay_id']").val() ;// 要购买的Id
    var price = $(this).data("price")
    var type_id = $(this).data("typeid") // 模型ID
      ...
      onBeforeOpen: () => {
        ...
        if (alipay) {
          alipay.addEventListener('click', () => {
            to_pay_post(alipay.dataset.id,type_id,post_id,price)
          })
        }
        if (weixinpay) {
          weixinpay.addEventListener('click', () => {
            to_pay_post(weixinpay.dataset.id,type_id,post_id,price)
          })
        }
        if (yecpay) {
          yecpay.addEventListener('click', () => {
            to_yecpay_post(yecpay.dataset.id,type_id,post_id,price)
          })
        }
